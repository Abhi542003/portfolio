...
import { playBeep, playStartup } from '../utils/soundEffects';
...
export const Hero = () => {
  ...
  const handleRippleClick = (e, callback) => {
    ...
    setButtonRipples((prev) => [...prev, { id, x, y, size }]);
    setTimeout(() => {
      setButtonRipples((prev) => prev.filter((r) => r.id !== id));
    }, 600);
  ...
  const socials = [
    { href: "https://github.com/Abhi542003/", icon: <FaGithub className="w-5 h-5" />, label: "GitHub" },
    { href: "http://linkedin.com/in/abhijit-purohit-a05117324", icon: <FaLinkedin className="w-5 h-5" />, label: "LinkedIn" },
  ...
  return (
    ...
    >
        ...
        <div className="fixed inset-0 z-[9997] flex flex-col items-center justify-center space-y-8 w-full h-full px-6 select-none pointer-events-auto">
          ...
          >
            ...
            <div className="relative w-full h-full rounded-full overflow-hidden border-2 border-slate-800 shadow-[0_20px_50px_rgba(168,85,247,0.35)] bg-black p-0.5 z-10">
              <img src={profilePic} alt="Abhijit Rajpurohit" className="w-full h-full rounded-full object-cover" />
            </div>
            {/* 🎓 Badge */}
            <div className="absolute bottom-[-8px] bg-slate-900/90 dark:bg-slate-950/90 border border-slate-700 py-1 px-3.5 rounded-full flex items-center space-x-1 shadow-2xl z-20 text-[10px] font-bold text-white select-none">
              <span>🎓 Fresher QA Engineer</span>
            </div>
          </motion.div>

          {/* Welcoming Tag Launch Button */}
      ...
      {/* PORTFOLIO HERO LAYOUT (Fades and morphs elements once opened) */}
      {isOpen && (
        <>
          ...
          <div className="w-full flex flex-col lg:flex-row items-center justify-between gap-12 z-10 my-auto">
            ...
            <div className="w-full lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left space-y-6 lg:pr-8">
              ...
              {/* Profile Image with layout transition morphing */}
              <motion.div
                layoutId="hero-profile-pic"
              ...
              >
                ...
                <div className="absolute bottom-[-8px] bg-slate-900/90 dark:bg-slate-950/90 border border-slate-700/60 dark:border-slate-800 py-1 px-3.5 rounded-full flex items-center space-x-1 shadow-[0_8px_20px_rgba(0,0,0,0.4)] z-20 text-[10px] font-bold text-white tracking-wide select-none">
                  <span>🎓 Fresher QA Engineer</span>
                </div>
              </motion.div>

              {/* Welcome tag morphed to column */}
              <motion.div
                layoutId="hero-welcome-btn"
              ...
              <div className="flex flex-col sm:flex-row items-center space-y-3.5 sm:space-y-0 sm:space-x-4 w-full sm:w-auto relative">
                ...
                >
                  ...
                  {/* Shine Sweep Overlay */}
                  <div className="absolute inset-0 w-[50%] h-full bg-gradient-to-r from-transparent via-white/20 to-transparent skew-x-[-25deg] translate-x-[-150%] group-hover:translate-x-[250%] transition-transform duration-1000 ease-out pointer-events-none z-0" />
                  {/* Ripple Circles */}
                  {buttonRipples.map((ripple) => (
                    <span
                      ...
                      className="absolute rounded-full bg-white/25 pointer-events-none animate-btn-ripple"
                    />
                  ))}
                </motion.a>
                ...
                <motion.a
                  ...
                  whileTap={{ scale: 0.96 }}
                  className="magnetic flex items-center justify-center space-x-2 py-3.5 px-8 rounded-xl font-bold glass-panel border border-slate-200 dark:border-slate-800/80 text-slate-700 dark:text-slate-355 hover:bg-slate-100 dark:hover:bg-slate-900/60 transition-all duration-300 w-full sm:w-auto group relative overflow-hidden pointer-events-auto"
                >
                  ...
                  {/* Shine Sweep Overlay */}
                  <div className="absolute inset-0 w-[50%] h-full bg-gradient-to-r from-transparent via-white/10 to-transparent skew-x-[-25deg] translate-x-[-150%] group-hover:translate-x-[250%] transition-transform duration-1000 ease-out pointer-events-none z-0" />
                  {/* Ripple Circles */}
                  {buttonRipples.map((ripple) => (
                    <span
                      key={ripple.id}
                      ...
                      className="absolute rounded-full bg-white/20 pointer-events-none animate-btn-ripple"
                    />
                  ))}
                </motion.a>
              </div>
              ...
              >
                {socials.map((social, idx) => (
                  <motion.a
                    ...
                    className="social-icon w-11 h-11 rounded-xl glass-panel flex items-center justify-center text-slate-500 hover:text-purple-600 dark:text-slate-400 dark:hover:text-purple-400 transition-all duration-300 pointer-events-auto"
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 max-w-7xl mx-auto">
              ...
              <motion.div
                ...
                className="flex items-center space-x-4 glass-panel border border-slate-200/50 dark:border-slate-800/80 rounded-3xl p-6 shadow-2xl relative overflow-hidden backdrop-blur-md transition-all duration-305"
              >
                <motion.div
